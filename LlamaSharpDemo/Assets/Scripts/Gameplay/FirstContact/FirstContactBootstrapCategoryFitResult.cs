...
{
    public static class FirstContactPromptJson
    {
    ...
    public sealed class FirstContactSemanticGroupFitResult
    {
        ...
        public static FirstContactSemanticGroupFitResult ResolveSeedMemberVerifications(
            string proposedCategory,
            IReadOnlyList<FirstContactSemanticGroupFitResult> memberResults)
        {
            string category = FirstContactSemanticCategory.Normalize(proposedCategory);
            ...
            {
                return Failed("Semantic group seed verification returned no member results.");
            }

            for (int i = 0; i < memberResults.Count; i++)
            {
                FirstContactSemanticGroupFitResult memberResult = memberResults[i];
                if (memberResult?.IsSuccess != true)
                {
                    return Failed(
                        memberResult?.Error ?? "Semantic group seed member verification failed.");
                }
