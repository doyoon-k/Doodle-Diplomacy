...
/// </summary>
public class RuntimeLlamaSharpService : MonoBehaviour, ILlmService, IEmbeddingService
{
    [Serializable]
    ...
    {
        if (!TryGetLocalProfile(settings ?? defaultProfile, out LlmGenerationProfile localSettings))
        {
            return null;
        }
        ...
        Action<string> onResponse)
    {
        ...
        BaseLlmGenerationProfile activeSettings = settings ?? localSettings;
        ...
        string systemPrompt = LlamaSharpInterop.RenderSystemPrompt(activeSettings, state);
        string prompt = LlamaSharpInterop.BuildUserPrompt(
            activeSettings,
            userPrompt,
            requiresJson: !string.IsNullOrWhiteSpace(activeSettings.format),
            systemPrompt: systemPrompt,
    ...
    private IEnumerator EnsureExecutorLoaded(BaseLlmGenerationProfile settings, Action<ILLamaExecutor> onReady)
    {
        if (_shutdownRequested)
        {
            onReady?.Invoke(null);
    ...
    private bool HasRunningOperations()
    {
        lock (_operationLock)
        {
            ...
            return inferenceRunning || embeddingRunning || loadRunning;
        }
    }
        ...
        out LlamaSharpEmbeddingRuntimeKey key)
    {
        ...
        if (profile == null)
        {
            Debug.LogError("[RuntimeLlamaSharpService] LlmEmbeddingProfile is missing.");
            return false;
        }
        ...
        if (string.IsNullOrWhiteSpace(resolvedModelPath))
        {
            Debug.LogError("[RuntimeLlamaSharpService] Embedding model path is empty.");
            return false;
        }

        if (!File.Exists(resolvedModelPath))
        {
            Debug.LogError($"[RuntimeLlamaSharpService] Embedding model file not found: {resolvedModelPath}");
            return false;
        }
        ...
        LlamaSharpEmbeddingRuntimeKey key)
    {
        lock (_embeddingRuntimes)
        {
            ...
            if (logTraffic)
            {
                Debug.Log($"[RuntimeLlamaSharpService] Loaded embedding model: {resolvedModelPath}");
            }
    ...
    private bool TryGetLocalProfile(BaseLlmGenerationProfile profile, out LlmGenerationProfile localProfile)
    {
        ...
        Debug.LogError($"[RuntimeLlamaSharpService] Unsupported profile type '{profile.GetType().Name}'. This service only handles local LlmGenerationProfile.");
        return false;
    }
