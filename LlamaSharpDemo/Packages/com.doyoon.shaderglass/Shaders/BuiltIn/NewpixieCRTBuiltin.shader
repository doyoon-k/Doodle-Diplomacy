Shader "Hidden/ShaderGlass/BuiltIn/Newpixie/CRT"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float3 tsample(sampler2D samp, float2 tc, float offs, float2 resolution)
            {
            ...
            fixed4 Frag(Varyings input) : SV_Target
            {
                ...
                if (use_frame > 0.5)
                {
                    float4 frame = tex2D(frametexture, input.uv);
                    float frameBlend = frame.a;
                    col = lerp(col, frame.rgb, frameBlend);
                }
