...
- `GameFlowDirector` is the additive gameplay-scene loader. It reads `GameFlowAsset`, loads a `FlowEntryDefinition` scene, finds an `IGameplaySceneInstaller`, and asks `GameplayModeHost` to enter the scene-provided mode.
- `FlowEntryDefinition` is the sequence unit. It can represent a day gameplay scene, report, interlude, interactive cutscene, minigame, event, ending, or any custom scene/mode pair.
- `SceneReferenceHub` is the current gameplay-scene installer and authoritative inspector-wired reference list for scene services. Do not create missing runtime components as fallbacks.
- `GameplayModeHost` configures `InteractionManager` through `SceneReferenceHub` so scene clicks route to the active mode.
- The current object-pair drawing gameplay is a direct `IGameplayMode`/`IGameplaySessionController` implementation on `RoundManager`. There is no adapter layer between the host and the current mode.
- `RoundManager` orchestrates the object-pair drawing mode only. It does not call AI pipeline implementation details directly.
- `RoundManager` delegates startup flow, state versioning, state entry routing, player action handling, input routing, inspector fallback interaction binding, UI hints, camera mode application, interaction gates, drawing locks, preview terminal presentation, configured text lookup, and intro sequence preparation to focused round services.
...
- The current object-pair drawing mode uses `ObjectPairDrawingInteractionPolicy`, which delegates to `InteractionStatePolicy`.
...
- UI controllers subscribe to `GameplayModeHost` state and invoke gameplay actions through `IGameplaySessionController`.
...
- `GameManager`
  ...
  - `RoundManager` (current object-pair drawing gameplay mode)
  - `ScoreManager`
  - `AIPipelineBridge` (external AI runtime)
  - `GamePipelineRunner`
- `RoundManager` references:
  - `IInteractionStateService`, `ICameraModeService`
  - `IDrawingFeature` injected from `GameplayModeContext`; serialized `DrawingBoardController` remains only as an inspector fallback.
  - Serialized `InteractionManager` and `CameraController` remain only as inspector fallbacks.
...
- `RoundManager` only binds direct `InteractableObject.OnInteracted` UnityEvents in debug scenes without a `GameplayModeHost` route.
