...
using DoodleDiplomacy.Narrative.Editor;
using UnityEditor;
using UnityEngine;
...
namespace DoodleDiplomacy.Editor
{
    public sealed class LocalizationWorkbenchWindow : EditorWindow
    {
        ...
        private void DrawExistingEntryDetail(LocalizationRow row)
        {
            ...
            bool generatedByNarrativeDesk = NarrativeGeneratedKeyRegistry.TryGet(
                key.stringValue,
                out NarrativeGeneratedKeyManifestEntry generatedEntry);
            if (generatedByNarrativeDesk)
            {
                EditorGUILayout.HelpBox(
                    $"Generated by Narrative Desk from {generatedEntry.scenarioId}" +
                    (string.IsNullOrWhiteSpace(generatedEntry.beatId)
                        ? "."
                        : $" / {generatedEntry.beatId}."),
                    MessageType.Info);
                if (GUILayout.Button("Open Source in Narrative Desk"))
                {
                    NarrativeGeneratedKeyRegistry.OpenSource(key.stringValue);
                }
            }

            using (new EditorGUI.DisabledScope(generatedByNarrativeDesk))
            {
        ...
        private static string CategorizeKey(string key)
        {
            ...
            if (key.StartsWith("first_contact.narrative.", StringComparison.OrdinalIgnoreCase) ||
                key.StartsWith("first_contact.doctor_hwang.", StringComparison.OrdinalIgnoreCase) ||
                key.StartsWith("first_contact.placeholder.", StringComparison.OrdinalIgnoreCase))
            {
                return "Narrative";
            }
